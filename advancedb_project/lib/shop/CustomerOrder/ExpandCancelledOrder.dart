...
class ExpandCancelledOrder extends StatefulWidget {
  ...
  @override
  State<ExpandCancelledOrder> createState() => _ExpandCancelledOrderState();
}

class _ExpandCancelledOrderState extends State<ExpandCancelledOrder> {
  ...
  Future<void> _fetchCancelledOrders() async {
    ...
    try {
      // Using both endpoints to ensure maximum compatibility
      final response = await http.get(
        Uri.parse('http://localhost:5000/api/orders?shop_id=${widget.shopData['id']}&status=declined'),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _cancelledOrders = List<Map<String, dynamic>>.from(data ?? []);
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildOrderField('User', order['user_name'] ?? 'Unknown User'),
              _buildOrderField('Service', order['service_name'] ?? 'Unknown Service'),
              _buildOrderField('Address', order['address'] ?? 'No address'),
              _buildOrderField('Date', _formatDate(order['created_at'])),
              _buildOrderField('Reason', order['cancel_reason'] ?? 'No reason provided'),
            ],
          ),
        ),
      ),
    ...
    return BottomNavigationBar(
      ...
      items: const [
        BottomNavigationBarItem(
          icon: ImageIcon(
            AssetImage('assets/ProfileScreen/Home.png'),
            color: Colors.grey,
          ),
          label: 'Home',
        ),
        BottomNavigationBarItem(
          icon: ImageIcon(
            AssetImage('assets/ProfileScreen/Orders.png'),
            color: Colors.grey,
          ),
          label: 'Orders',
        ),
        BottomNavigationBarItem(
          icon: ImageIcon(
            AssetImage('assets/ProfileScreen/Services.png'),
            color: Colors.grey,
          ),
          label: 'Services',
        ),
